...
export function fetchUniversityWeather(university) {
  ...
  const average = temps.then(arr => arr.length ? arr.reduce((acc, e)=> acc+e, 0)/arr.length : 0);
  return Promise.all([temps, universities, average]).then((arr) => arr[1].reduce((acc, e, i) => ({...acc, [e]: arr[0][i] }),{totalAverage:arr[2]})).catch(err => console.log(err));
}
